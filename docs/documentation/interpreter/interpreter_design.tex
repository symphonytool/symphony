...
\node[draw=none] (env) {};
\node[draw,circle,right=2.2cm of env] (parallel1) {$W[|\{inp\}|]R$};
\path[draw,dashed,-open triangle 60] (env) edge node[above]{inspect} node[below]{\{tau(par-begin)\}} (parallel1);

\node[draw,circle, right=4cm of parallel1] (parallel2) {$[|\{inp\}|]$};
...
begin rule which does the initialization needed. In the figure that
rule is named tau(par-begin) and is therefore returned from the
inspection. The reason for the name tau(..) is that transitions can be
observable or silent, so in principle any tau transition is not seen
from the outside of the process. In the interpreter however all
transitions flows out of the inspection. When the inspection phase has
completed, the execution phase begins. The execution phase executes
one of the transitions returned from the inspection phase. In this
case, only a single transition is available so the tau(par-begin)) is
executed which creates the two child processes. The result of each of
the shown steps are the first configuration shown in the next step. So
in this case the resulting process configuration is shown in figure
\autoref{fig:cmlmodel_second_step}.


The second step shown in \autoref{fig:cmlmodel_second_step} has a
little more interesting inspection phase. According to the parallel
composition rule for this construct, we have that any event on the inp
channel must be synchronized, meaning that W and R must perform any
transition that involves event on the inp channel synchronously. 
...
\node[draw=none] (env) {};
\node[draw,circle,right=2cm of env] (parallel1) {$[|\{in\}|]$};
\node[draw,circle,below left=1.5cm and .2 cm of parallel1] (W1) {$inp!1$};
\node[draw,circle,below right=1.5cm and .2 cm of parallel1] (R1) {$inp?x$};
...
\node[draw,circle, right=4cm of parallel1] (parallel2) {$[|\{in\}|]$};
\node[draw,circle,below left=1.5cm and .2 cm of parallel2] (W2) {$inp!1$};
\node[draw,circle,below right=1.5cm and .2 cm of parallel2] (R2) {$inp?x$};
...
event. This is then given to the execution phase which performs this
event and moves both child processes into the next state.
...
\node[draw=none] (env) {};
\node[draw,circle,right=2cm of env] (parallel1) {$[|\{in\}|]$};
\node[draw,circle,below left=1.5cm and .2 cm of parallel1] (W1) {$Skip$};
\node[draw,circle,below right=1.5cm and .2 cm of parallel1] (R1) {$out!1$};
\path[draw,dotted,-open triangle 60] (parallel1) edge node[above left] {inspect} (W1);
...
\node[draw,circle, right=4cm of parallel1] (parallel2) {$[|\{in\}|]$};
\node[draw,circle,below left=1.5cm and .2 cm of parallel2] (W2) {$Skip$};
\node[draw,circle,below right=1.5cm and .2 cm of parallel2] (R2) {$out!1$};
...
\node[draw,circle,right=2cm of env] (parallel1) {$[|\{in\}|]$};
\node[draw,circle,below left=1.5cm and .2 cm of parallel1] (W1) {$Skip$};
\node[draw,circle,below right=1.5cm and .2 cm of parallel1] (R1) {$Skip$};
...
\node[draw,circle, right=3cm of parallel1] (parallel2) {$[|\{in\}|]$};
\node[draw,circle,below left=1.5cm and .2 cm of parallel2] (W2) {$Skip$};
\node[draw,circle,below right=1.5cm and .2 cm of parallel2] (R2) {$Skip$};
...
\node[draw,circle, right=1.5cm of parallel2] (parallel3) {$Skip$};
