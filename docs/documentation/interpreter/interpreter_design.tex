...
\draftnote{I can't remember the reason behind the two methods instead of one, find out!} 

\subsubsection{ConcreteCmlBehavior}
...
explained in details. To understand the dynamic model we need to see
what happens in the inspect and execute methods, as these together
determines the possible transitions at the top level show in the last
section.

On \autoref{fig:behavior_inspect_seq} the general inspect dynamics is depicted. When inspect is called on a ConcreteCmlBehavior it uses its nextNode (in the java source nextNode and nextContext is actually a Pair<INode, Context> call)
\begin{figure}[ht!]
  \begin{center}
