...
public class ChoiceStopLeft implements IRefineLaw {
	...
	public boolean isApplicable(INode node) {
		...
		if (node instanceof AActionStm) {
			...
			if (n1.getAction() instanceof AExternalChoiceAction) {
				return ((AExternalChoiceAction) n1.getAction()).getLeft() instanceof AStopAction;			
			}
		}
		return false;
	}
	...
	public Refinement apply(INode node) {
		return new Refinement(((AExternalChoiceAction) (((AActionStm) node).getAction())).getRight().toString(), new LinkedList<PExp>());
	}	
