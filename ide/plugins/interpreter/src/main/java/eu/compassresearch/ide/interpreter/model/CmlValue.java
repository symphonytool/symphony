...
public class CmlValue extends CmlDebugElement implements IVdmValue,
		IIndexedValue
{
	...
	public static CmlValue createValue(CmlStackFrame frame,
			IDbgpProperty property)
	{
		IVdmType type = createType(frame.getDebugTarget(), property);
		return new CmlValue(frame, property, type);
	}
	...
	protected CmlValue(CmlStackFrame frame, IDbgpProperty property,
			IVdmType type)
	{
		this.frame = frame;
		this.type = type;
	...
	{
		int offset = 0;// getPageOffset(page);
		...
		if (size > 0)
		{
			for (int i = 0; i < size; ++i)
			{
				...
				variables[offset + i] = new CmlVariable(frame, p.getName(), p);
			}
	...
	/*
	 * @see org.eclipse.dltk.debug.core.model.CmlValue#getMemoryAddress()
	 */
