...
IContentOutlinePage
{
	...
	public void setTreeSelection(INode element)
	{
		...
		if (dscr == null)
		{
			ILexNameToken name = pdef.getName();
			w = Wrapper.newInstance(pdef, name == null ? "null"
					: name.getName());
		} else
						...
						if (o instanceof Wrapper)
						{
							...
							for (Method m : w.value.getClass().getMethods())
							{
								if ("getLocation".equals(m.getName()))
								{
									try
									{
										LexLocation loc = (LexLocation) m.invoke(w.value, new Object[0]);
										if (loc == null)
											return;
										editor.setHighlightRange(loc.startOffset, 0, true);
									} catch (Exception e)
									{
										e.printStackTrace();
									}
