...
public class DefinitionMap
{
	...
	private static Map<Class<?>, ? extends DefinitionHandler> createMap()
	{
		...
		map.put(AFunctionsDefinition.class, new AFunctionParagraphDefinitionHandler());
		map.put(ATypesDefinition.class, new ATypesParagraphDefinitionHandler());
		map.put(AOperationsDefinition.class, new AOperationParagraphDefinitionHandler());
	...
	private static class AValuesDefinitionHandler implements DefinitionHandler
	{
				...
				PDefinition pdef)
		{
			...
			for (PDefinition subdef : ((AValuesDefinition) pdef).getValueDefinitions())
			{
				String typeguard = "?";
			...
			DefinitionHandler
	{
		...
		{
			List<Wrapper<? extends PDefinition>> r = new LinkedList<Wrapper<? extends PDefinition>>();
			for (PDefinition subdef : ((AFunctionsDefinition) pdef).getFunctionDefinitions())
			{
			...
			DefinitionHandler
	{
		...
		{
			List<Wrapper<? extends PDefinition>> r = new LinkedList<Wrapper<? extends PDefinition>>();
			for (SCmlOperationDefinition subdef : ((AOperationsDefinition) pdef).getOperations())
				r.add(Wrapper.newInstance(subdef, subdef.getName().getName()
						+ ": " + subdef.getType()));
			return r;
		}
