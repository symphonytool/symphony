...
public class Path {
	public enum PathKind {
		UNIT, TILDE, DOT, DOT_MATCHVALUE, BANG_MATCHVALUE, BACKTICK, DOTHASH, APPLY, SEQRANGE;
	}
	...
	public PAction convertToAction() throws PathConvertException {
		...
		case UNIT: {
			...
			action = new ACallAction(location, actionName);
		}
			break;
		// case DOT:
		// {
		...
		case APPLY: {
			...
			action = new ACallStatementAction(location, objectDesignator,
					pair.second, this.expList);
		}
			break;
		default:
	...
	public ANameChannelExp convertToChannelNameExpression()
			throws PathConvertException {
	...
	public PExp convertToExpression() throws PathConvertException {
		...
		case TILDE: {
			...
			if (name instanceof AVariableExp) {
				...
				throw new PathConvertException(
						"Illigal path for old name expression");

		}
			break;
		case DOT: {
			PExp root = this.subPath.convertToExpression();
			exp = new AFieldExp(location, root, null/*
													 * LexNameToken
													 * memberName_???
													 */, this.unit.value);
		}
		...
		case BACKTICK:
			if (this.subPath.kind == PathKind.UNIT) {
				LexNameToken name = this.unit.convertToName(subPath.unit.value
						.getName());
				// exp = new ANameExp(location,name);
				exp = new AVariableExp(location, name, "");
			} else {
		...
		case DOTHASH: {
			...
			exp = new ATupleSelectExp(null/* PType type_ */, location, tuple,
					numeral);
		}
			break;
		case APPLY: {
			PExp root = this.subPath.convertToExpression();
			exp = new AApplyExp(location, root, expList);
		}
			break;
		case SEQRANGE: {
			...
			if (this.expList.size() != 2)
				throw new PathConvertException(
						"A subset sequence expression must have 2 expression arguments");

			exp = new ASubseqExp(location, seq, this.expList.get(0),
					this.expList.get(1));
		}
			break;
		}
	...
	public PStateDesignator convertToStateDesignator()
			throws PathConvertException {
		if (unit != null && unit.kind == Unit.UnitKind.SELF)
			throw new PathConvertException(
					"stateDesignators cannot contains self expressions");

		PStateDesignator sd = null;
		switch (kind) {
		case UNIT: {
			LexNameToken name = unit.convertToName();
			sd = new AIdentifierStateDesignator(location, name);
		}
			break;
		case DOT: {
			PStateDesignator object = this.subPath.convertToStateDesignator();
			sd = new AFieldStateDesignator(location, object, unit.value);
		}
			break;
		case BACKTICK: {
		...
		case APPLY: {
			...
			if (this.expList.size() != 1)
				throw new PathConvertException(
						"A map or sequence reference must have 1 argument");
			sd = new AMapSeqStateDesignator(location, object,
					this.expList.get(0));
		}
			break;
		default:
			throw new PathConvertException(
					"Illigal path for stateDesignator : " + kind);
		}
	...
	public PProcess convertToProcess() throws PathConvertException {
		...
		case UNIT: {
			...
			process = new AInstantiationProcess(location, null, name, null,
					null);
		}
			break;
		// case DOT:
		...
		case APPLY: {
			...
			if (pair.first != null) {
				throw new PathConvertException(
						"Illigal path for instantiation proces : ");
			}
	...
	public PObjectDesignator convertToObjectDesignator()
			throws PathConvertException {
		...
		case UNIT: {
			...
			if (this.unit.kind == Unit.UnitKind.IDENTIFIER)
				od = new AIdentifierObjectDesignator(location,
						new LexNameToken(module, this.unit.value), null);
			else if (this.unit.kind == Unit.UnitKind.SELF)
				od = new ASelfObjectDesignator(location, new LexNameToken(
						module, this.unit.value));

		}
			break;
		case DOT: {
			od = new AFieldObjectDesignator(location,
					this.subPath.convertToObjectDesignator(), null, /*
																	 * LexNameToken
																	 * className_
																	 */
					this.unit.value);
		...
		case BACKTICK: {
			od = new AIdentifierObjectDesignator(location,
					this.convertToName(), null);
		}
			break;
		case APPLY: {
			od = new AApplyObjectDesignator(location,
					this.subPath.convertToObjectDesignator(), this.expList);
		}
			break;
		default:
			throw new PathConvertException(
					"Illigal path for objectDesignator : " + kind);
		}
	...
	private Pair<LexNameToken, List<PExp>> convertToChannelNameExpHelper()
			throws PathConvertException {
		Pair<LexNameToken, List<PExp>> pair = null;
		switch (kind) {
		case UNIT: {
			pair = new Pair(convertToName(), new LinkedList<PExp>());

		}
			break;
		case DOT: {
			Pair<LexNameToken, List<PExp>> tmpPair = this.subPath
					.convertToChannelNameExpHelper();
			LexNameToken name = unit.convertToName();
		...
		default:
			throw new PathConvertException(
					"path not implemented for channel expression " + kind);
		}
		return pair;
	}
	...
	// names: ['.'] id
	// ['.'] id`id
	private Pair<Path, LexNameToken> extractPostfixName()
			throws PathConvertException {
		LexNameToken name = null;
		Path path = null;
		switch (kind) {
