...
public class CmlInterpreterStateDTO
{
	...
	public static CmlInterpreterStateDTO createCmlInterpreterStateDTO(
			CmlInterpreter activeInterpreter)
	{
		return new CmlInterpreterStateDTO(activeInterpreter.getTopLevelProcess(), activeInterpreter.getState(), activeInterpreter.getActiveBreakpoint(), new LinkedList<TransitionDTO>());
	}

	public static CmlInterpreterStateDTO createCmlInterpreterStateDTO(
			CmlInterpreter activeInterpreter, List<TransitionDTO> events)
	{
		return new CmlInterpreterStateDTO(activeInterpreter.getTopLevelProcess(), activeInterpreter.getState(), activeInterpreter.getActiveBreakpoint(), events);
	}

	private static List<CmlProcessDTO> convertProcessTreeIntoList(
	...
	public CmlInterpreterStateDTO(CmlBehaviour topProcess,
	...
	public CmlInterpreterStateDTO(CmlBehaviour topProcess,
			CmlInterpreterState state, Breakpoint bp,
	...
	{
		return "CmlInterpreterState: "
				+ state
				+ System.lineSeparator()
				+ "topProcess: "
				+ (this.topLevelProcess != null ? this.topLevelProcess.toString()
						: "NA");
	}
