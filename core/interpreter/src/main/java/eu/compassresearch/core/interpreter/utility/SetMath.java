...
//	}
	...
	private static void getCrossProduct(SetValue result, List<SetValue> sets, int depth, List<Value> current)
	{
	...
	private static void getCrossProduct(List<List<Value>> results, List<SetValue> sets, int depth, List<Value> current)
	{
		for (int i = 0; i < sets.get(depth).values.size(); i++)
		{
			List<Value> tmp = new LinkedList<Value>(current);
