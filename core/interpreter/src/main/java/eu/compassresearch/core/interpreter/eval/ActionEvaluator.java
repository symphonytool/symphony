...
//			newQuestion.setCurrentEvent(new CMLChannelEvent(""));
	...
	public Value caseASequentialCompositionAction(
		...
		/*
		 * If retValue == null, then this means that the left side 
		 * is Skip and the action is now the right action 
		 */
		if(retValue == null )
		{
			CMLContext newQuestion = new CMLContext(node.getLocation(),"", question);
//			newQuestion.setCurrentEvent(new CMLChannelEvent(""));
	...
	/*
	 * The internal choice action 'A |~| B' behaves nondeterministically as either A or B.
	 * Operational semantics: A silent transition (tau) into either A or B
	 */
	@Override
	public Value caseAInternalChoiceAction(AInternalChoiceAction node,
		 ...
		 * just always pick the right one. This of course need to be more "randomish"
		 * deterministic
		 */
	...
	public Value caseAGeneralisedParallelismParallelAction(
		...
		if( (leftValue != null && !leftValue.isSkip()) || (rightValue != null && !rightValue.isSkip()))
		{
			...
			/*
			 * Find all the events that are allowed
			 * Event is cs: Both left and right must be able to sync on this
			 * Event is not in cs: the event can be carried out independent of the other action
			 */
			...
			{
				//Check for event in channelset and leftSync, if not in both its a no go
				if(chanset.contains(e.getIdentifier().getName()) && 
