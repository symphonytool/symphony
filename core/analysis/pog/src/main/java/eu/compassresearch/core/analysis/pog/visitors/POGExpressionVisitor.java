...
public class POGExpressionVisitor extends
	...
	CMLProofObligationList obligations = node.getExp()
		.apply(this, question);
	...
	try {
	    obligations.addAll(defaultSNumericBinaryExp(
		    (SNumericBinaryExp) node, question));
	...
	if (!(rExp instanceof AIntLiteralExp)
	    ...
	    obligations.add(new CMLNonZeroObligation(node.getLocation(), rExp,
		    question));
	}
