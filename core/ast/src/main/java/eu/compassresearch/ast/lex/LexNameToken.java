...
public class LexNameToken extends LexIdentifierToken implements ILexNameToken, Serializable
  {
    ...
    public LexNameToken getOldName()
      {
        return new LexNameToken(module, new LexIdentifierToken(name, true,
            location));
      }
    ...
    public LexNameToken getNewName()
      {
        return new LexNameToken(module, new LexIdentifierToken(name, false,
            location));
      }
        ...
        if (typeQualifier != null && lother.getTypeQualifier() != null)
          {
            ...
            try
              {
                Class helpLexNameTokenClass = cls
                    .loadClass("org.overture.typechecker.util.HelpLexNameToken");
            ...
            throw new InternalException(-1,
                "Use HelpLexNameToken.isEqual to compare");
        ...
        if (hashcode == 0)
          {
            hashcode = module.hashCode()
                + name.hashCode()
                + (old ? 1 : 0)
                + (typeQualifier == null ? 0 : PTypeAssistant
