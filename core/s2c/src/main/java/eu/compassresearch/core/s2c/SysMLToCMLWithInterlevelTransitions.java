	...
	public String translate(State s, State p)
	{
		...
		if (s.substates.isEmpty())
		{
			...
			if (s.name.startsWith("Final"))
			{
				 ...
				 * because they are only executed if that state is reached. Stop is a unity of external choice,
				 * therefore the completion transitions can happen. Furthermore, they can be interrupted by the
