...
public class ProcessCmlTypeCheckerTestCase extends AbstractTypeCheckerTestCase
{
	static
	{
		...
		// 2//
		add("process A = begin @ Skip end process B = begin @ Skip end process p1 = A [| channel1 |] B", false); // negative
																													// as
																													// channel1
																													// is
		// 3//
		add("process A = begin @ Skip end process B = begin @ Skip end process p1 = A [ channel1 || channel2 ] B", false); // negative
																															// as
																															// channel1
																															// and
		// 4//
		...
		// 5//
		add(3, "process A = begin @ Skip end process p1 = begin @ (A \\\\ {| B |}) end", true, false, new String[] { TypeErrorMessages.EXPECTED_AN_ACTION.customizeMessage("A") });
		// 6//
		add(3, "process A = begin @ Skip end process p1 = begin @ (A \\\\ {| channel1 |}) end", true, false, new String[] { TypeErrorMessages.EXPECTED_AN_ACTION.customizeMessage("A") });
		// 7//
		...
		// 8//
		add(3, "process A = begin @ Skip end process p1 = begin @ (A startsby 42) end", true, false, new String[] { TypeErrorMessages.EXPECTED_AN_ACTION.customizeMessage("A") });
		// 9//
		add(3, "process A = begin @ Skip end process p1 = begin @ (A endsby 42) end", true, false, new String[] { TypeErrorMessages.EXPECTED_AN_ACTION.customizeMessage("A") });
		...
		// 14//
		add("process K = begin state a:int operations o: int ==> int o(b) == if a = 0 then a:=b+1 elseif 1 then a := b - a else a := 0 ; return a @ a := o(9) end", true, false);
		// 15//
		add("process K = begin state a:int operations o: int ==> int o(b) == if a = 0 then a:=b+1 elseif a > 1 then a:='a' else a := 0 ; return a @ a := o(9) end", true, false);
		// 16//
		add("process K = begin state a:int operations o: int ==> int o(b) == if a = 0 then a:=b+1 elseif a > 1 then a:=b-a else a := 'l' ; return a @ a := o(9) end", true, false);
		// 17//
		add("process K = begin state a:int operations o: int ==> int o(b) == if a = 0 then a:=b+1 elseif a > 1 then a:=b-a else a := 0 ; return a @ a := o('l') end", true, false);
		// 18//
		add("process A = begin actions B = val n:int @ Skip @ (||| i in set {1,2,3} @ [ { } ] B(i)) end");
		// 19//
		add("process A = begin @ A [[ init <- start ]] end ");
		// 20//
		add("process K = begin operations A:int*int ==> bool A(a,b) == return (a=b) @ A(2) end", true, false);
		// 21//
		add("process L = begin operations A:int*int*int ==> bool A(a,b,c) == o(0);return (a+b=c) @ A(1,mk_(0,2)) end", true, false);
		// 22//
		add("process L = begin state k : int operations K:int*int ==> int K(a,b) == for all i in set {1,2,3} do k := k + o @ Skip end", true, false);
		// 23//
