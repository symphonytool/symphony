...
public class CmlTypeCheckerTestCase extends AbstractTypeCheckerTestCase {
	...
	public static Collection<Object[]> parameter() {
		...
		addTestProgram(testData, 
				...
				true, true, new String[0]);
		//63 //
		addTestProgram(testData, "channels	a : int	process A =	val x : nat @ begin	@ a.x -> Skip end process B  = |~| i in set {1,2,3} @ A(i)", 
				true, true, new String[0]);
		//64 //
				addTestProgram(testData, "channels	a : int	process A =	val x : nat @ begin	@ a.x -> Skip end process B = ; i in seq {1,2,3} @ A(i)", 
						true, true, new String[0]);
