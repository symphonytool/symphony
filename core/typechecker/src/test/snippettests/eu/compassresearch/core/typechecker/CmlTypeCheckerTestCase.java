...
public class CmlTypeCheckerTestCase extends AbstractTypeCheckerTestCase
{
	...
	public static Collection<Object[]> parameter()
	{
		...
		addTestProgram(testData, "class T = begin end class N = begin end types TorN = T | N", true, true, true, new String[0]);
		// 1
		addTestProgram(testData, "process T = begin operations o:int*int ==> int o(a,b) == return a+b @ o(10,10) end", true, true, true, new String[0]);
		// 2
		addTestProgram(testData, "types I = int process T = begin operations o:I*I ==> int o(a,b) == return a+b @ o(10,10) end", true, true, true, new String[0]);
		// 3
		addTestProgram(testData, "types R :: a : int b : int " + "process P = "
				+ "  " + " " + "begin " + "  state " + "    a : int "
				+ "    k : R   " + "  actions "
				+ "    A = cases k : mk_R(a1,b1) -> a:=a1+b1 end" + " @ A "
				+ "end", true, true, true, new String[0]);
		// 4
		addTestProgram(testData, "types Value = int ID = nat process P = begin operations CheckMac: Value * Value * ID ==> bool CheckMac(a,b,c) == return (a*b=c) Me: () ==> bool Me() == (dcl a : Value := 2 @ return CheckMac(a,2,4)) @ Skip end ", true, true, true, new String[0]);
		// 5
		addTestProgram(testData, "channels out: nat1 process S1 = || k in set {1,2,3} @ [{ }] begin @ out!k -> Skip end", true, true, false, new String[0]);
		// 6
		addTestProgram(testData, "channels c1:int*int process A = begin state a:int @ [| {  } |] i in set {1,2,3} @ [{ c1 }] c1!i -> Skip end", true, false, true, new String[0]);
		// 7
		addTestProgram(testData, "channels c1:int*int process A = begin state a:int @ [| {a } |] i in set {1,2,3} @ [{    }] c1!i -> Skip end", true, false, true, new String[0]);
		// 8
		addTestProgram(testData, "channels c1:int*int process A = begin state a:int @ [| { c1 } |] i in set {1,2,3} @ [{  a }] c1!i -> Skip end", true, false, true, new String[0]);
		// 9
		addTestProgram(testData, "channels c1:int*int process A = begin state a:int @ [| { } |] i in set {1,2,3} @ [{   }] Skip end", true, true, false, new String[0]);
		// 10
		addTestProgram(testData, "channels c1:int*int process A = begin state a:int @ [| {c1, a} |] i in set {1,2,3} @ [{   }] c1!i -> Skip end", true, false, true, new String[0]);
		// 11
		addTestProgram(testData, "channels c1:int*int process A = begin state a:int @ [| { } |] i in set {1,2,3} @ [{  c1, a }] c1!i -> Skip end", true, false, true, new String[0]);
		// 12
		addTestProgram(testData, "channels c1:int*int process A = begin state a:int @ [| { } |] i in set {1,2,3} @ [{  a  }] c1!a -> Skip end", true, false, true, new String[0]);
		// 13
		addTestProgram(testData, "channels c1:int*int process A = begin state a:int @ [| { c1 } |] i in set {1,2,3} @ [{   }] c1!a -> Skip end", true, false, true, new String[0]);
		// 14
		addTestProgram(testData, "process T = begin operations O: () ==> () O() == Skip @ O() end", true, true, true, new String[0]);
		// 15
		addTestProgram(testData, "process A = begin @ let b = 2 in [b = 2] & Skip end", true, true, true, new String[0]);
		// 16
		addTestProgram(testData, "values a : int = 42 process A = begin values b : int = 43 @Skip end", true, true, true, new String[0]);
		// 17
		addTestProgram(testData, "channels c process A = begin @ ||| k in set {1,2,3} @ [{ }]c!k -> Skip end", true, false, true, new String[0]);
		// 18
		addTestProgram(testData, "channels c:nat1 process A = begin @ ||| k in set {1,2,3} @ [{ }]c!k -> Skip end", true, true, true, new String[0]);
		// 19
		addTestProgram(testData, "process K = begin actions INIT = Skip @ INIT() end", true, true, true, new String[0]);
		// 20
		...
		// 23
		addTestProgram(testData, "channels a: int types book = token values mbook = { mk_token(\"Book\") |-> 1} process P = begin @  a!(mbook(book))->Skip end", true, true, true, new String[0]);
		// 24
		...
		// 42
		addTestProgram(testData, "class C = begin operations C:()==>C C() == Skip end process P = begin state c:C @ c := new C() end", true, true, true, new String[0]);
		// 43
		addTestProgram(testData, "process A = begin state i:int := 0 m:map int to (map int to int) @ m(0)(0) := 1 end", true, true, true, new String[0]);
		// 44
