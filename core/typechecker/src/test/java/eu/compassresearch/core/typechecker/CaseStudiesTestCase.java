 ...
 * 
 * Use the environment variables {@code CASESTUDIES} to find the
 * directory where the case studies are located. If this environment 
 * variable is not located all tests are skipped.
 * 
 * @author rwl
 *
 */
...
public class CaseStudiesTestCase extends AbstractTypeCheckerTestCase {
	...
	private static String caseStudyDir = (String)System.getenv().get("CASESTUDIES");
	 ...
	 * compasssvn/Common/CaseStudies/{@value pathRelToCaseStudiesDir} add a
	 * test including all cml-sources of the case study.
	 * 
	 * E.g.:
	 * add("SoSMpc/ideal") or 
	...
	static {
		if(canFindCaseStudies) {
			...
			add("Dwarf");
			//2// one file has a parse error others - fails due to LexNameToken conflict
